...
# FIXME: check if the first condition being relative to mean isn't preventing an early exit (i dont think it is)
...
class NBVD_coclustering:
    ...
    def attempt_coclustering(self, R,B,C,Z, save_history=False):
        ...
        while (i == 0 or abs(current_norm - previous_norm) > 0.001*Z.mean() or abs(current_norm - previous_norm) > 0.001*previous_norm or abs(current_norm - previous_norm) > previous_norm/higher_dim**2) and i < NBVD_coclustering.ITER_MAX and current_norm <= previous_norm:
    ...
    def attempt_coclustering_sym(self, S,B,Z):
        ...
        while (i == 0 or abs(current_norm - previous_norm) > 1 or abs(current_norm - previous_norm) > 0.001*previous_norm) and i < NBVD_coclustering.ITER_MAX:
    ...
    def do_things(self, Z, rng, verbose=False, save_history=False):
        ...
        while attempt_no < self.n_attempts:
            ...
            if verbose:
                if iter_stop < NBVD_coclustering.ITER_MAX:
